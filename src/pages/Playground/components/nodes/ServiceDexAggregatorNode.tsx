...
interface DexAggregatorNodeData {
    label: string;
...
export const ServiceDexAggregatorNode = memo(({ data }: { data: DexAggregatorNodeData }) => {
    ...
    const getStatusIcon = () => {
        switch (data.status) {
            ...
            default: return <TrendingUp className="h-4 w-4 text-emerald-600" />;
        }
    };
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center gap-2 mb-2">
                {getStatusIcon()}
                <div>
                    <div className="font-semibold text-sm">DEX Aggregator</div>
            ...
            {data.output && (
                <div className="text-xs space-y-1">
                    <div className="font-semibold text-green-700">
                        Best: {data.output.bestVenue}
                    </div>
                    <div className="text-gray-600">
                        Price: {data.output.bestPrice.toFixed(4)}
                    </div>
                    <div className="text-gray-500">
                        {data.output.quotes.length} venues checked
                    </div>
                </div>
            )}
...
ServiceDexAggregatorNode.displayName = 'ServiceDexAggregatorNode';
