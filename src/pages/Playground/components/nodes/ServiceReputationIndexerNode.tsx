...
interface ReputationIndexerNodeData {
    label: string;
    status: 'idle' | 'executing' | 'complete' | 'error';
    output?: { reputationScore: number; trend: string; metrics: any };
}

export const ServiceReputationIndexerNode = memo(({ data }: { data: ReputationIndexerNodeData }) => {
    ...
    return (
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            <div className="flex items-center gap-2 mb-2">
                {getStatusIcon()}
                <div>
                    <div className="font-semibold text-sm">Reputation Indexer</div>
                    <div className="text-xs text-gray-500">{data.label}</div>
                </div>
            </div>
            {data.output && (
                <div className="text-xs space-y-1">
                    <div className="text-gray-600">Score: {data.output.reputationScore}/100</div>
                    <div className="text-gray-600">Trend: {data.output.trend}</div>
                </div>
            )}
...
ServiceReputationIndexerNode.displayName = 'ServiceReputationIndexerNode';
