...
interface RwaProofNodeData {
    label: string;
    status: 'idle' | 'executing' | 'complete' | 'error';
    output?: { verificationResult: any; slaMetrics: any };
}

export const ServiceRwaProofNode = memo(({ data }: { data: RwaProofNodeData }) => {
    ...
    return (
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            <div className="flex items-center gap-2 mb-2">
                {getStatusIcon()}
                <div>
                    <div className="font-semibold text-sm">RWA Proof Submission</div>
                    <div className="text-xs text-gray-500">{data.label}</div>
                </div>
            </div>
            {data.output && (
                <div className="text-xs space-y-1">
                    <div className="text-gray-600">Proof verified</div>
                </div>
            )}
...
ServiceRwaProofNode.displayName = 'ServiceRwaProofNode';
