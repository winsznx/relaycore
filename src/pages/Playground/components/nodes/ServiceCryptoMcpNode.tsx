...
interface CryptoMcpNodeData {
    label: string;
    status: 'idle' | 'executing' | 'complete' | 'error';
    output?: { marketData: any; orderbook: any };
}

export const ServiceCryptoMcpNode = memo(({ data }: { data: CryptoMcpNodeData }) => {
    ...
    const getStatusIcon = () => {
        switch (data.status) {
            ...
            default: return <Coins className="h-4 w-4 text-indigo-600" />;
        }
    };
    ...
    return (
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            <div className="flex items-center gap-2 mb-2">
                {getStatusIcon()}
                <div>
                    <div className="font-semibold text-sm">Crypto.com MCP</div>
                    <div className="text-xs text-gray-500">{data.label}</div>
                </div>
            </div>
            {data.output && (
                <div className="text-xs space-y-1">
                    <div className="text-gray-600">Market data loaded</div>
                </div>
            )}
...
ServiceCryptoMcpNode.displayName = 'ServiceCryptoMcpNode';
