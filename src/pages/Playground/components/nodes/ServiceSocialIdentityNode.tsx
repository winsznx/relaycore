...
interface SocialIdentityNodeData {
    label: string;
    status: 'idle' | 'executing' | 'complete' | 'error';
    output?: { walletAddress: string; verified: boolean; platform: string };
}

export const ServiceSocialIdentityNode = memo(({ data }: { data: SocialIdentityNodeData }) => {
    ...
    const getStatusIcon = () => {
        switch (data.status) {
            ...
            default: return <Users className="h-4 w-4 text-rose-600" />;
        }
    };
    ...
    return (
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            <div className="flex items-center gap-2 mb-2">
                {getStatusIcon()}
                <div>
                    <div className="font-semibold text-sm">Social Identity</div>
                    <div className="text-xs text-gray-500">{data.label}</div>
                </div>
            </div>
            {data.output && (
                <div className="text-xs space-y-1">
                    <div className="text-gray-600">{data.output.platform}</div>
                    <div className={data.output.verified ? 'text-green-700' : 'text-gray-600'}>
                        {data.output.verified ? '✓ Verified' : 'Not verified'}
                    </div>
                </div>
            )}
...
ServiceSocialIdentityNode.displayName = 'ServiceSocialIdentityNode';
